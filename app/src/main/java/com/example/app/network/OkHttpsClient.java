...
public class OkHttpsClient {
    ...
    public Response doLogin(String username, String password) {
        try {
            ...
            RequestBody body = new MultipartBody.Builder().setType(MultipartBody.FORM).addFormDataPart("username", username).addFormDataPart("password", password).build();
            Request request = new Request.Builder().url("http://143.42.66.73:9090/public/api/login.php").method("POST", body).build();
